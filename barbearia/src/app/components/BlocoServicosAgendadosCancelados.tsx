...
export default function BlocoServicosAgendadosCancelados() {
  ...
  const buscarServicosCancelados = useCallback(async (periodo: Periodo) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    buscarServicosCancelados(periodo);
  }, [periodo, buscarServicosCancelados]);
  ...
  return (
    <section className="bg-white border border-gray-200 rounded-xl p-6 flex flex-col gap-4 min-h-[140px] shadow-md w-full">
      <div className="flex items-center gap-3 mb-2">
        <FaCut className="text-red-600 text-2xl" />
      ...
      <span className="text-sm text-gray-600 font-semibold uppercase tracking-wide mb-2">Serviços Cancelados</span>
